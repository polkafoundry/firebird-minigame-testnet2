...
const { signGiftCodeBackend } = require("./util");
...
describe("GameContract", function () {
  ...
  beforeEach(async () => {
    ...
    console.log("1");
    const TokenTest = await ethers.getContractFactory("BirdToken");
    ...
    const GiftCodeContract = await ethers.getContractFactory("SBirdGiftCode");

    giftCodeContract = await (
      await upgrades.deployProxy(
        // @ts-ignore
        GiftCodeContract,
        [token.address, fundWallet.address],
        { initializer: "__SBirdGiftCode_init" }
      )
    ).deployed();
    console.log("Gift code contract deployed to:", betContract.address);
    await giftCodeContract.connect(deployer).setSigner(signer.address);
    ...
    await token.connect(fundWallet).approve(giftCodeContract.address, MAX_INT);
  });
  ...
  describe("Claim gift code Token ", function () {
    it("should  withdraw token with signature", async () => {
      const sig2 = await signGiftCodeBackend(
        giftCodeContract,
        gamer1.address,
        deployer.address,
        "abcd",
      ...
      await giftCodeContract.connect(gamer1).useCode("abcd", "1000", sig2);
      expect(parseFloat(utils.formatEther(await token.balanceOf(gamer1.address)))).to.eq(1000);
    });
  });
});
