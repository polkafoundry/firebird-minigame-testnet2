...
interface INFTEscrow {
  ...
  event NFTWithdrawed(string transactionId, uint256[] tokenIds, address indexed user, uint256 deadline);
  event TokenDeposited(uint256 amount, address indexed user);
  event TokenWithdrawed(string transactionId, uint256 amount, address indexed user, uint256 deadline);

  event NFTSelledToShop(uint256[] tokenIds, string[] rarity, address indexed user, uint256 deadline);
}
