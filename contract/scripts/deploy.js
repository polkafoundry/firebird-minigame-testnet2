...
async function main() {
  ...
  // --- Gift Code contract
  const GiftCodeContractFactory = await hre.ethers.getContractFactory("SBirdGiftCode");
  const GiftCodeContract = await upgrades.deployProxy(
    GiftCodeContractFactory,
    ["0x52774D49Edd34402a4ACF61b4074Bdf3c3B961B0", process.env.FUND_ADDRESS],
    {
      initializer: "__SBirdGiftCode_init",
    }
  );
  const giftCodeContract = await GiftCodeContract.deployed();
  console.log("Gift code contract deployed to:", giftCodeContract.address);

  //set signer
  const setSignerGiftCode = await giftCodeContract.setSigner(process.env.SIGNER_ADDRESS.toLowerCase());
  console.log("set signer", setSignerGiftCode.hash);
}
