...
export default class CalcOddsHtJob implements JobContract {
  ...
  public async handle(job) {
    ...
    const match = await MatchModel.query().where('is_calculated_odds_ht', false).where('is_half_time', true).first()
    console.log('CalcOddsHtJob: ', match)
    if (!match) return

    calcBettingJob({ matchId: match.id, betType: Const.BET_TYPE.ODDS_HT })
  }
}
