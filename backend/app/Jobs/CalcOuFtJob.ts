...
export default class CalcOuFtJob implements JobContract {
  ...
  public async handle(job) {
    ...
    const match = await MatchModel.query().where('is_calculated_ou_ft', false).where('is_full_time', true).first()
    console.log('CalcOuFtJob: ', match)
    if (!match) return

    calcBettingJob({ matchId: match.id, betType: Const.BET_TYPE.OU_FT })
  }
}
