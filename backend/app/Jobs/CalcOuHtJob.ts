...
export default class CalcOuHtJob implements JobContract {
  ...
  public async handle(job) {
    ...
    const match = await MatchModel.query().where('is_calculated_ou_ht', false).where('is_half_time', true).first()
    console.log('CalcOuHtJob: ', match)
    if (!match) return

    calcBettingJob({ matchId: match.id, betType: Const.BET_TYPE.OU_HT })
  }
}
